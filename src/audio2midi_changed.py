...
class Audio2Midi:
    ...
    def transition_matrix(self, note_min, note_max, p_stay_note, p_stay_silence):
        """
    ...
    def probabilities(
        ...
        f0, voiced_flag, voiced_prob = librosa.pyin(
            y, fmin * 0.9, fmax * 1.1, sr, frame_length, window_length, hop_length
        )
        tuning = librosa.pitch_tuning(f0)
        f0_ = np.round(librosa.hz_to_midi(f0 - tuning)).astype(int)
        onsets = librosa.onset.onset_detect(
            y, sr=sr, hop_length=hop_length, backtrack=True
        )
    ...
    def states_to_pianoroll(self, states, note_min, note_max, hop_time):
        """
    ...
    def pianoroll_to_midi(self, y, pianoroll):
        """
    ...
    def run(self, file_in, file_out):
        ...
        T = self.transition_matrix(note_min, note_max, 0.9, 0.2)
        ...
        # print(states)
        pianoroll = self.states_to_pianoroll(
            states, note_min, note_max, hop_length / sr
        )
        # print(pianoroll)
        MyMIDI = self.pianoroll_to_midi(y, pianoroll)
...
if __name__ == "__main__":
    ...
    audio_2_midi.run(file_in, file_out)
